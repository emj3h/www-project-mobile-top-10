...
Threat agents who exploit insecure cryptography in mobile applications can undermine the confidentiality, integrity, and authenticity of sensitive information. These threat agents include skilled adversaries who target cryptographic algorithms or implementations to decrypt sensitive data, malicious insiders who manipulate cryptographic processes or leak encryption keys, state-sponsored actors engaged in cryptanalysis for intelligence purposes, cybercriminals who exploit weak encryption to steal valuable data or conduct financial fraud, and third-party adversaries who leverage vulnerabilities in cryptographic protocols or libraries. These threat agents exploit insecure cryptography by employing various techniques such as brute force attacks, cryptographic attacks, side-channel attacks, or exploiting implementation flaws. It is essential for mobile application developers to adopt strong cryptographic algorithms, proper key management, and secure implementation practices to mitigate the risks posed by these threat agents and protect the integrity and security of sensitive data.

# Attack vectors

**Exploitability Average**

Any API call that is accessible to potential attackers can be exploited as an attack vector. An attack vector refers to a specific method or pathway through which an adversary can gain unauthorized access, manipulate data, or compromise the security of the mobile application.
...
**Insecure Storage of Data or Encryption Keys**: If the encryption keys are stored insecurely on the mobile device, such as in plain text or in easily accessible locations, attackers with physical or unauthorized access to the device can retrieve the keys and decrypt the protected data.
If the mobile application utilizes a weak encryption algorithm or uses encryption incorrectly, such as using a weak key or failing to properly encrypt all sensitive data. This can result in compromised data if the encryption is easily bypassed or decrypted by an attacker.
